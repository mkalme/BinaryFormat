...
namespace BinaryFormatV2 {
    public class BObject : BTag, IEnumerable {
        ...
        public BObject(int capacity){
            _tags = new Dictionary<string, BTag>(capacity);
        }
        ...
        public static BObject FromBytes(byte[] bytes) {
            FastBinaryReader reader = new FastBinaryReader(bytes);

            return Deserialize(reader);
        }
        public static BObject FromStream(FastBinaryReader reader){
            return Deserialize(reader);
        }
        internal static BObject Deserialize(FastBinaryReader reader){
        ...
        public byte[] ToBytes(){
            using (MemoryStream m = new MemoryStream()) {
                using (FastBinaryWriter writer = new FastBinaryWriter(m)) {
                    Serialize(writer);
                }
                return m.ToArray();
            }
        }
        public void ToStream(FastBinaryWriter writer) {
            Serialize(writer);
        }
