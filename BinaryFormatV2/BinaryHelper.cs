...
namespace BinaryFormatV2 {
    static class BinaryHelper {
        //Conversion
        public static BTag ReadTag(Type type, FastBinaryReader reader) {
            ...
            if (type == typeof(BArray)) return BArray.Deserialize(reader);
            if (type == typeof(BObject)) return BObject.Deserialize(reader);

            return null;
        }
        public static void WriteTag<T>(this T obj, FastBinaryWriter writer) {
            switch (obj) {
                case sbyte sb:
    ...
    static class BitExtensions {
        ...
        public static T[] To<T>(this Array array) {
            return (T[])array;
        }
    }
}